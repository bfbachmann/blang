...
/// Performs semantic analysis on all the given modules and returns the result of analysis.
pub fn analyze_modules(modules: Vec<Module>) -> ProgramAnalysis {
    let mut ctx = ProgramContext::new();
    let mut module_errors: Vec<HashMap<Position, AnalyzeError>> = vec![];
    let mut module_warns: Vec<HashMap<Position, AnalyzeWarning>> = vec![];

    // First pass: define types and functions in the program without analyzing them yet.
    for module in &modules {
        define_types(&mut ctx, module);
        define_fns(&mut ctx, module);
        define_consts(&mut ctx, module);

        // Take all warnings and errors from the program context, replacing them with empty maps.
        module_errors.push(std::mem::take(&mut ctx.errors));
        module_warns.push(std::mem::take(&mut ctx.warnings));
    }

    // Second pass: fully analyze all program statements.
    let mut analyzed_modules = vec![];
    for (i, module) in modules.iter().enumerate() {
        let analyzed_module = AModule::from(&mut ctx, module);

        // Perform move checks and add any errors to our list of errors only if semantic analysis
        // passed.
        if ctx.errors().is_empty() {
            let errors = MoveChecker::check_prog(&analyzed_module, ctx.type_store());
            for err in errors {
                ctx.insert_err(err);
            }
        }
        ...
        // Take all warnings and errors from the program context, replacing them with empty maps.
        module_errors[i].extend(std::mem::take(&mut ctx.errors));
        module_warns[i].extend(std::mem::take(&mut ctx.warnings));
    }

    let mut results: Vec<AnalyzedModule> = vec![];
    for module in analyzed_modules {
        let errors = module_errors.remove(0);
        let warns = module_warns.remove(0);

        results.push(AnalyzedModule::new(module, errors, warns));
    }
