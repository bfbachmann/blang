...
impl<'a: 'ctx, 'ctx> ModuleCodeGen<'a, 'ctx> {
    ...
    fn gen_extern_fn(&mut self, ext_fn: &AExternFn) {
        ...
        // nocommit: Support C and non-C calling conventions.
        ...
        let ll_intern_fn = self
            .ll_mod
            .add_function(&mangled_name, ll_ext_fn_type, linkage);
        let is_sret = fn_sig
            .maybe_ret_type_key
        ...
        for (i, arg) in fn_sig.args.iter().enumerate() {
            let mut ll_arg = ll_ext_fn.get_nth_param((i + param_offset) as u32).unwrap();
